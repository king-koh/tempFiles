...
onnx_path = "/Lidar_AI_Solution/CUDA-BEVFusion/qat/byUsingConfigs/head.bbox.fp32.sim.onnx"
model = onnx.load(onnx_path)
...
# -----------------------------
def list_compiler_funcs_any(mod_p, compiler_name="cgc"):
    hit = []
    for gv, f in mod_p.functions.items():
        if not isinstance(f, relay.Function):
            continue
        name = getattr(gv, "name_hint", str(gv))
        s = f.astext(show_meta_data=False)
        # attrs見落とし対策: 文字列に compiler_name が出るかでも拾う
        if (f.attrs is not None and "Compiler" in f.attrs.keys() and str(f.attrs["Compiler"]) == compiler_name) or (compiler_name in s):
            hit.append((name, f))
    return hit
...
txt = mod.astext(show_meta_data=False)
print("len(mod.astext) =", len(txt))
print("contains topk? ", "topk" in txt)
print("contains matmul? ", "matmul" in txt.lower())
print("contains nn.batch_matmul? ", "nn.batch_matmul" in txt)
print("contains softmax? ", "softmax" in txt.lower())
